...
const Hero = () => {
    return (
        <section className="relative min-h-screen flex items-center overflow-hidden bg-[#030712]">
            ...
            {/* Background Effects */}
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,rgba(77,42,131,0.15),transparent_90%)]"></div>

            {/* Video positioned on the right side - smaller and more to the right */}
            <div className="absolute top-1/2 right-10 -translate-y-1/2 w-[50%] h-[70%] z-0 hidden lg:block">
                <video
                    autoPlay
                    loop
                    muted
                    ...
                    style={{
                        maskImage: 'radial-gradient(ellipse 70% 70% at 60% 50%, black 20%, transparent 90%)',
                        WebkitMaskImage: 'radial-gradient(ellipse 70% 70% at 60% 50%, black 20%, transparent 60%)',
                        opacity: 0.6
                    }}
                >
                    ...
                    <div className="flex flex-col items-start text-left py-20">
                        ...
                        <div className="inline-flex items-center bg-white/5 backdrop-blur-md border border-white/10 rounded-full px-5 py-2 mb-8 hover:bg-white/10 transition-colors cursor-default">
                            <span className="flex h-2 w-2 relative mr-3">
                                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-[#83a1cb] opacity-75"></span>
                                <span className="relative inline-flex rounded-full h-2 w-2 bg-[#83a1cb]"></span>
                            </span>
                            ...
                            {heroData.buttons.map((btn, index) => (
                                btn.primary ? (
                                    <Link
                                        key={index}
                                        href={btn.link}
                                        className="group relative px-8 py-4 rounded-full font-semibold text-sm tracking-wider text-white bg-[#83a1cb] hover:bg-[#569dff] transition-all duration-300 shadow-lg shadow-purple-900/30 hover:shadow-cyan-500/30 flex items-center justify-center"
                                    >
                                        {btn.text}
